 ...
 protected:
  ...
  bool IsBlockedBerth(geometry_msgs::PoseStamped const& berth_pose) {
    ...
    Eigen::Vector3d half_size(cfg_.Get<double>("blocked_berth_size_x") / 2.0,
                              ...
                              cfg_.Get<double>("blocked_berth_size_z") / 2.0);
    Eigen::Vector3d goal_position(berth_pose.pose.position.x,
                                  berth_pose.pose.position.y,
                                  berth_pose.pose.position.z);

    Eigen::Quaterniond orientation(berth_pose.pose.orientation.w,
                                   berth_pose.pose.orientation.x,
                                   berth_pose.pose.orientation.y,
                                   berth_pose.pose.orientation.z);
    if (orientation.norm() < 1e-6) {
      orientation = Eigen::Quaterniond::Identity();
    } else {
      orientation.normalize();
    }
    Eigen::Vector3d cargo_position =
      goal_position + orientation * Eigen::Vector3d(0.0, 0.255, 0.0);

    auto inside = [&center, &half_size](Eigen::Vector3d const& position) {
      Eigen::Vector3d delta = (position - center).cwiseAbs();
      return delta.x() <= half_size.x() &&
             delta.y() <= half_size.y() &&
             delta.z() <= half_size.z();
    };

    // Checking both positions also rejects clients that accidentally provide
    // the cargo body pose instead of the documented berth pose.
    return inside(goal_position) || inside(cargo_position);
  }
  ...
  void GoalCallback(isaac_msgs::CargoGoalConstPtr const& goal) {
    ...
    case isaac_msgs::CargoGoal::DROP:
    {
      ...
      if (IsBlockedBerth(goal_.pose)) {
        return Result(RESPONSE::BERTH_FULL,
                      "The requested cargo berth is blocked");
      }
